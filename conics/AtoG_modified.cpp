...
void AtoG_modified(M6x1 ParAin, M5x1 &ParG, int &code)
/*        <- Input ->  <----- Output ------>
  ...
  Conversion of Algebraic parameters of a conic to its Geometric parameters
   ...
   4. Intersecting lines:  canonical equation  (cos(u)*x+sin(u)*y+d)*(cos(v)*x+sin(v)*y+e) = 0
    ...
    Beyond code 4 written by Michael William Hentrich, February 2019

       5. Single Point:  canonical equation  x^2/a^2 + y^2/b^2 = 0
                 a>b or a=b are the major and minor semi-axes

       ParG = [Xcenter, Ycenter, a, b]'
*/
{
    ...
    if (Ten+det33 == Ten)         //  if the big matrix is singular...
    {
        ...
        {
            code = 8;    //  single point
            // MICHAEL WILLIAM HENTRICH hard coding completing the square:
            int gcd;
            if (fabs(ParAin(3) - 0.0) < 0.000001)  ParG(3) = gcd = 1; 
            else {
              gcd = gcf((int)ParAin(0), (int)ParAin(3));
              ParG(3) = sqrt((reals)gcd); // b
            }
            ParG(0) = -(ParAin(3)/gcd);   // Xcenter
            if (fabs(ParAin(4) - 0.0) < 0.000001)  ParG(2) = gcd = 1; 
            else  {
               gcd = gcf((integers)ParAin(2), (integers)ParAin(4));
               ParG(2) = sqrt((reals)gcd);  // a
            }
            ParG(1) = -(ParAin(4)/gcd);   // Ycenter
            return;
        }
        else
        {
...
//     Next: non-degenrate types of conics
    ...
    if ((Ten+D2(0)==Ten)||(Ten+D2(1)==Ten))
    {
        ...
        if (abs(D2(0))>abs(D2(1)))
        {
            ...
            if (p<0.)
            {
              //  p = -p;          // added by MW Hentrich for a2g.cpp
                Angle = Angle + pi;
            }
        ...
        else
        {
            ...
            Center = Q22*Uc;                    //   Center of the parabola
            p = -U(0)/D2(1);                    // the distance from focus to directrix (line missing in some files)
