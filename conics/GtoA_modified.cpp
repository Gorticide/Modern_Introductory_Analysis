#include<cmath>
 void GtoA_modified(M6x1 &ParA, M5x1 ParG, int code, char orient = 'v')
/*        <- Output -><----- Input ----->
...
*/
{
    ...
    reals LAST,c,s,a,b,p,c1,s1,c2,s2,Xc,Yc,P,Q,R,One=1.,Two=2.;
...
if (code == 1)     //   ellipse
{
    ...
    if (fabs(R - 0.0) < 0.00001) LAST = P*Xc*Xc+Q*Yc*Yc-P*Q;
    else LAST = P*Xc*Xc+Q*Yc*Yc+2.*R*Xc*Yc-1.;
     ParA << P, R, Q, -P*Xc-R*Yc, -Q*Yc-R*Xc, LAST;
   // ParA.normalize();
}
...
if (code == 3)     //   parabola
{
  if (orient == 'v' || orient == 'V') {    // switch cosine and sine for vertical axis
      c = cos(ParG(3));  s = sin(ParG(3));  // when angle is 0, c = 1, s = 0
      p = ParG(2);
      Xc = ParG(0);  Yc = ParG(1);
      R = Xc*c - Yc*s;  
      if (fabs(R - 0.0) < 0.00001) LAST = 2.*p*(Xc*s+Yc*c);
      else LAST = R*R+2.*p*(Xc*s+Yc*c);
      ParA << c*c, c*s, s*s, -R*c-p*s, R*s-p*c, LAST;    // we need x^2 coefficient but not y^2
    }
    else   {
    ...
    Xc = ParG(0);  Yc = ParG(1);
    R = Xc*s - Yc*c;  
    if (fabs(R - 0.0) < 0.00001) LAST = 2.*p*(Xc*c+Yc*s);
    else LAST = R*R+2.*p*(Xc*c+Yc*s);
    ParA << s*s, c*s, c*c, -R*10*s-p*c, R*c-p*s, LAST;  // we need y^2 coefficient but not x^2
    //ParA.normalize();
    }
 ...
 if (code == 4)      //   intersecting lines
  ...
  //  ParA << c1*c2, (c1*s2+c2*s1)/Two, s1*s2, (c1*ParG(3)+c2*ParG(1))/Two, (s1*ParG(3)+s2*ParG(1))/Two, ParG(1)*ParG(3);
   // ParA.normalize();
   /* CHANGED FORM from ParG = [u, d, v, e] 
       to ParG = [Xcenter, Ycenter, a, b, AxisOfRotation = 0]
       */
    reals X, Y, a, b;
    X = ParG(0);
    Y = ParG(1);
    ...
    if (a < 0)   // x^2/a^2 - y^2/b^2 = 0 ----> b^2*x^2 - a^2*y^2 = 0
      ParA << b*b, 0.0, -a*a, -X*b*b, Y*a*a, X*X*b*b - Y*Y*a*a;
    else if (b < 0)  // y^2/b^2 - x^2/a^2 = 0 ----> a^2*y^2 - b^2*x^2 = 0
      ParA << -b*b, 0.0, a*a, X*b*b, -Y*a*a, Y*Y*a*a - X*X*b*b;
