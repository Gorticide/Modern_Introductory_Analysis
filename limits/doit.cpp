...
int main()   {
    ...
    for (int i = 1; i < 100; ++i)  {
        if (abs(a(i) - L) >= E)  {
          M = i;
          cout << "n = " << i << "; a(" << i << ") = " << a(i) << "; |" 
               << a(i) << " - 2| = " << abs(a(i) - L) << endl;
         }
     }
    cout << "\nThe value of M such that all terms of the sequence\n"
         << "{a_n} = { " << eqn << " } with n >= M \n"
         << "are in the neighborhood of the limit " << L 
         << " of radius " << E << " is: " << M + 1 << endl;
    return 0;
}


double a(int n)   {
    //return (double)(2*n*n)/(n*n + 4);   // Computer Exercise 6
...
are in the neighborhood of the limit 2 of radius 0.1 is: 8
...
are in the neighborhood of the limit 2 of radius 0.1 is: 19
*/
